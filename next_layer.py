...
from api import WeightsEnum, Weights, _ovewrite_named_param
...
if __name__ == "__main__":
    ...
    def predict_image(input_batch):
        ...
        with torch.no_grad():
            output = model(input_batch)  # .logits  # aux_logits1 logits, aux_logits2
    ...
    og_model = torch.hub.load(
        "pytorch/vision:v0.10.0", "googlenet", pretrained=True
    ).eval()
    print(og_model.inception4a.forward(0))
    model = googlenet(weights=og_model.parameters()).eval()
    input_image = Image.open(
        "data/ILSVRC2015/Data/DET/test/ILSVRC2012_test_00000003.JPEG"
    )
